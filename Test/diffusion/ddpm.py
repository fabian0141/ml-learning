...
def train(args):
    ...
    if args.start > 0:
        ckpt = torch.load(os.path.join("models", args.run_name, f"ckpt.pt"))
    ...
    for epoch in range(args.start ,args.epochs):
        ...
        for i, (data, _) in enumerate(pbar):
            ...
            #print(f"MSE {loss.item()} global_step={epoch * l + i}")

        print(f"Epochs {epoch}/{args.epochs}")
