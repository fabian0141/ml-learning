...
def run(path, index):
    match(mode):
        case 0:
            runCustomOpenGL(path, index)
        case 1:
            runOpen3D(path, index)
                
def runOpen3D(path, index):
    names = os.listdir(path)
    voxels = np.load(path + "\\" + names[index])
...
def runCustomOpenGL(path, index):
    names = os.listdir(path)

    game = Game()
    camera = Camera(Camera.PERSPECTIVE_VIEW)
    visualizer = VisualizeVoxels()
    voxels = np.load(path + "\\" + names[index])
    voxels = voxels.astype(np.float32)
    adjustSize(voxels)
    
    visualizer.showVoxels(voxels, names[index])
